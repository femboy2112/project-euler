...
# /masters — *know how to solve every problem that has been solved*

The Euler loop is this plugin's operating discipline; the masters' wing is what you reach for
when the problem's signature calls for a different documented skillset. This command runs ONE
lens, deliberately, with receipts. The full repertoire — each master's unique skillset in
general form, its dispatch table, and its boundary lessons — lives in the `the-masters` skill;
...
- **Empty** → dispatch on whatever is already on the table; if nothing is, say so plainly.

## The pass

1. **Load the lens.** Engage the `the-masters` skill; hold the chosen master's move-set in
   general form — the named operations, not the anecdotes.
2. **Dispatch honestly.** If the Euler loop itself is the right tool, say so and run it — a
   lens is a move, not a costume, and lens-shopping mid-stride is procrastination wearing a
   powdered wig.
...
- **The Ledger binds every lens.** Demonstrated / Observed / Conjectured / Refuted /
  UNVERIFIED — no lens output ships unlabeled, and numerics never silently upgrade.
- **The anti-patterns are refused, not reenacted:** the route goes in the record (no erased
  slate), findings surface when ripe (no hoard), certainty is a status pending verification
  (no missing label).
...
Close with: which lens ran and why; what it produced, each claim Ledger-labeled with exact
numbers and its boundary; what would upgrade or kill the best candidate; and the natural next
move (`/refute` for the favorite, `daniel` for calibration, `/basel` for the full arc, or the
plain Euler loop if the lens has done its job). In both cases we may learn something useful.
